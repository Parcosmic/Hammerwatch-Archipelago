...
def get_campaign(multiworld: MultiWorld, player: int) -> Campaign:
    return Campaign(get_option(multiworld, player, OptionNames.goal) // 10)


def get_goal_type(multiworld: MultiWorld, player: int) -> GoalType:
    return GoalType(get_option(multiworld, player, OptionNames.goal) % 10)
...
def get_active_key_names(multiworld: MultiWorld, player: int) -> typing.Set[str]:
    ...
    if campaign == Campaign.Castle:
        if get_option(multiworld, player, OptionNames.act_specific_keys):
            ...
            if get_option(multiworld, player, OptionNames.randomize_bonus_keys):
                key_names.update({
